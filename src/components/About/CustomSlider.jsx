...
const CustomSlider = ({ slides }) => {
  ...
  return (
    <div className="relative h-[1000px] pt-24 overflow-y-auto">
      ...
      {slides.map((slide, index) => (
        ...
        >
          {index === currentSlide && (
            <div className="h-[35vh] flex flex-col justify-center items-center pt-2 xl:pt-24">
              <img src={slide.imgSrc} alt={slide.imgAlt} className="w-[200px] xl:w-[500px] lg:w-[350px] md:w-[700px] sm:w-[250px] rounded-lg h-[170px] xl:h-[500px] lg:h-[500px] md:h-[400px] sm:[300px]" />
              <div className="text-white p-4">
                <h2 className="xl:text-4xl lg:text-2xl md:text-lg sm:text-sm text-center py-1 font-bold">{slide.title}</h2>
                <p className='mx-6 xl:mx-24 lg:mx-16 sm:mx-2 text-xs xl:text-xl lg:text-lg md:text-md sm:text-sm'>{slide.description}</p>
              </div>
            </div>
          )}
        </div>
